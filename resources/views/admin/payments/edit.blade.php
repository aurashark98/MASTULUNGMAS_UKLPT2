<x-admin-layout>
    <div class="max-w-2xl mx-auto space-y-8 animate-fade-in">
        ...
        <div>
            ...
            <h2 class="text-3xl font-black font-poppins text-gray-800 dark:text-white">Edit Transaksi #{{ $payment->id }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Perbarui nominal, metode pembayaran, status, atau ID transaksi.</p>
        </div>
        ...
        <div class="bg-white dark:bg-mtm-dark-surface p-8 rounded-[2.5rem] border border-gray-200 dark:border-white/5 shadow-sm">
            <form method="POST" action="{{ route('admin.payments.update', $payment) }}" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <x-text-input id="amount" class="block w-full !py-3.5 bg-gray-50/50 dark:bg-[#121212]/30 border-gray-100 dark:border-white/5 rounded-2xl" type="number" name="amount" :value="old('amount', intval($payment->amount))" required placeholder="Masukkan jumlah nominal transaksi" />
                ...
                <div class="space-y-2">
                    ...
                    <x-text-input id="payment_method" class="block w-full !py-3.5 bg-gray-50/50 dark:bg-[#121212]/30 border-gray-100 dark:border-white/5 rounded-2xl" type="text" name="payment_method" :value="old('payment_method', $payment->payment_method)" required placeholder="Contoh: Transfer Bank, E-Wallet, Qris" />
                ...
                <div class="space-y-2">
                    ...
                    <select id="status" name="status" required class="block w-full bg-gray-50/50 dark:bg-[#121212]/30 border-gray-200 dark:border-white/5 rounded-2xl focus:ring-mtm-red focus:border-mtm-red text-sm p-4 text-gray-700 dark:text-gray-300">
                        ...
                        <option value="completed" {{ old('status', $payment->status) == 'completed' ? 'selected' : '' }}>Completed (Sukses)</option>
                ...
                <!-- Transaction ID -->
                <div class="space-y-2">
                    <x-input-label for="transaction_id" :value="__('ID Transaksi *')" class="text-xs font-bold uppercase tracking-widest text-gray-500" />
                    <x-text-input id="transaction_id" class="block w-full !py-3.5 bg-gray-50/50 dark:bg-[#121212]/30 border-gray-100 dark:border-white/5 rounded-2xl" type="text" name="transaction_id" :value="old('transaction_id', $payment->transaction_id)" required placeholder="Contoh: MTM-123456789" />
