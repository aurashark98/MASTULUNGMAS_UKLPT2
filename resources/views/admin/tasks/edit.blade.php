<x-admin-layout>
    <div class="max-w-2xl mx-auto space-y-8 animate-fade-in">
        ...
        <div>
            ...
            <h2 class="text-3xl font-black font-poppins text-gray-800 dark:text-white">Edit Tugas: {{ $task->title }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Perbarui informasi tugas, kategori, anggaran, atau status pengerjaan.</p>
        </div>
        ...
        <div class="bg-white dark:bg-mtm-dark-surface p-8 rounded-[2.5rem] border border-gray-200 dark:border-white/5 shadow-sm">
            <form method="POST" action="{{ route('admin.tasks.update', $task) }}" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <x-text-input id="title" class="block w-full !py-3.5 bg-gray-50/50 dark:bg-[#121212]/30 border-gray-100 dark:border-white/5 rounded-2xl" type="text" name="title" :value="old('title', $task->title)" required placeholder="Contoh: Perbaikan AC Rusak di Rumah" />
                ...
                <div class="space-y-2">
                    ...
                    <x-text-input id="budget" class="block w-full !py-3.5 bg-gray-50/50 dark:bg-[#121212]/30 border-gray-100 dark:border-white/5 rounded-2xl" type="number" name="budget" :value="old('budget', intval($task->budget))" required placeholder="Masukkan angka anggaran" />
                ...
                <div class="space-y-2">
                    ...
                    <x-text-input id="location" class="block w-full !py-3.5 bg-gray-50/50 dark:bg-[#121212]/30 border-gray-100 dark:border-white/5 rounded-2xl" type="text" name="location" :value="old('location', $task->location)" required placeholder="Contoh: Lowokwaru, Malang" />
                ...
                <div class="space-y-2">
                    ...
                    <select id="status" name="status" required class="block w-full bg-gray-50/50 dark:bg-[#121212]/30 border-gray-200 dark:border-white/5 rounded-2xl focus:ring-mtm-red focus:border-mtm-red text-sm p-4 text-gray-700 dark:text-gray-300">
                        <option value="pending" {{ old('status', $task->status) == 'pending' ? 'selected' : '' }}>Pending (Mencari Mitra)</option>
                        <option value="assigned" {{ old('status', $task->status) == 'assigned' ? 'selected' : '' }}>Assigned (Mitra Ditugaskan)</option>
                        <option value="completed" {{ old('status', $task->status) == 'completed' ? 'selected' : '' }}>Completed (Selesai)</option>
                        <option value="cancelled" {{ old('status', $task->status) == 'cancelled' ? 'selected' : '' }}>Cancelled (Dibatalkan)</option>
                    </select>
