    ...
    <div class="glass rounded-full px-4 py-2.5 md:px-6 md:py-3 flex items-center justify-between shadow-2xl border border-black/20 dark:border-white/20">
        ...
        <div class="hidden lg:flex items-center gap-1">
            ...
            @foreach($navLinks as $link)
                <template x-if="activeNav === '{{ $link['id'] }}'">
                    <a href="{{ $link['url'] }}" 
                       class="px-5 py-2 rounded-full text-sm font-bold transition-all duration-300 relative group overflow-hidden text-mtm-red dark:text-mtm-red-light">
                        <span class="relative z-10">{{ $link['name'] }}</span>
                        <div class="absolute bottom-0 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-mtm-red rounded-full opacity-100"></div>
                    </a>
                </template>
            @endforeach
        </div>
