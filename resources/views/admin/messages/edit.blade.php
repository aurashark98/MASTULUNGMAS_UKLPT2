<x-admin-layout>
    <div class="max-w-2xl mx-auto space-y-8 animate-fade-in">
        ...
        <div>
            ...
            <h2 class="text-3xl font-black font-poppins text-gray-800 dark:text-white">Edit Pesan #{{ $message->id }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Perbarui isi pesan, pengirim, ruang obrolan, atau status baca pesan ini.</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white dark:bg-mtm-dark-surface p-8 rounded-[2.5rem] border border-gray-200 dark:border-white/5 shadow-sm">
            <form method="POST" action="{{ route('admin.messages.update', $message) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <select id="chat_room_id" name="chat_room_id" required class="block w-full bg-gray-50/50 dark:bg-[#121212]/30 border-gray-200 dark:border-white/5 rounded-2xl focus:ring-mtm-red focus:border-mtm-red text-sm p-4 text-gray-700 dark:text-gray-300">
                        @foreach($rooms as $room)
                            <option value="{{ $room->id }}" {{ old('chat_room_id', $message->chat_room_id) == $room->id ? 'selected' : '' }}>
                ...
                <div class="space-y-2">
                    ...
                    <select id="sender_id" name="sender_id" required class="block w-full bg-gray-50/50 dark:bg-[#121212]/30 border-gray-200 dark:border-white/5 rounded-2xl focus:ring-mtm-red focus:border-mtm-red text-sm p-4 text-gray-700 dark:text-gray-300">
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('sender_id', $message->sender_id) == $user->id ? 'selected' : '' }}>
                ...
                <div class="space-y-2">
                    ...
                    <textarea id="message" name="message" rows="4" placeholder="Tuliskan isi pesan obrolan..." class="block w-full bg-gray-50/50 dark:bg-[#121212]/30 border-gray-200 dark:border-white/5 rounded-2xl focus:ring-mtm-red focus:border-mtm-red text-sm p-4 text-gray-700 dark:text-gray-300">{{ old('message', $message->message) }}</textarea>
                    <x-input-error :messages="$errors->get('message')" class="mt-1" />
                </div>

                <!-- Image -->
                <div class="space-y-2">
                    <x-input-label for="image" :value="__('Lampiran Gambar (Kosongkan jika tidak diubah)')" class="text-xs font-bold uppercase tracking-widest text-gray-500" />
                    @if($message->image_path)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $message->image_path) }}" alt="Foto Lampiran" class="w-48 h-auto rounded-2xl object-cover border border-gray-200 dark:border-white/10 shadow-sm" />
                        </div>
                    @endif
                ...
                <div class="space-y-2">
                    ...
                    <select id="is_read" name="is_read" required class="block w-full bg-gray-50/50 dark:bg-[#121212]/30 border-gray-200 dark:border-white/5 rounded-2xl focus:ring-mtm-red focus:border-mtm-red text-sm p-4 text-gray-700 dark:text-gray-300">
                        <option value="0" {{ old('is_read', $message->is_read ? '1' : '0') == '0' ? 'selected' : '' }}>Belum Dibaca (Terkirim)</option>
                        <option value="1" {{ old('is_read', $message->is_read ? '1' : '0') == '1' ? 'selected' : '' }}>Sudah Dibaca</option>
                    </select>
