<x-admin-layout>
    <div class="space-y-8 animate-fade-in">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-3xl font-black font-poppins text-gray-800 dark:text-white">Kelola Penawaran Mitra</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Daftar penawaran harga yang diajukan oleh mitra untuk berbagai tugas.</p>
            </div>
            
            <a href="{{ route('admin.task-bids.create') }}" class="px-5 py-3 bg-gradient-to-r from-red-500 to-amber-500 hover:scale-[1.02] hover:shadow-lg hover:shadow-red-500/20 active:scale-[0.98] transition-all text-white font-bold text-sm rounded-2xl flex items-center gap-2 cursor-pointer shadow-md">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-gray-400 dark:text-gray-500 text-xs uppercase font-bold border-b border-gray-100 dark:border-white/5">
                            ...
                            <th class="px-6 py-4">Mitra (Penawar)</th>
                            <th class="px-6 py-4">Jumlah Penawaran</th>
                            <th class="px-6 py-4">Pesan Mitra</th>
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                        @forelse($bids as $bid)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <p class="font-bold text-sm text-gray-800 dark:text-white leading-tight mb-0.5">{{ $bid->task->title ?? 'Tugas Terhapus' }}</p>
                                    <p class="text-[10px] text-gray-400 dark:text-gray-500 font-medium">Anggaran Klien: Rp {{ number_format($bid->task->budget ?? 0, 0, ',', '.') }}</p>
                                </td>
                                <td class="px-6 py-4 text-sm font-bold text-gray-700 dark:text-gray-300">
                                    {{ $bid->mitra->name ?? 'Mitra Terhapus' }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-gray-500/10 text-gray-600 dark:text-gray-400">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.task-bids.edit', $bid) }}" class="p-2 bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 rounded-xl transition-all cursor-pointer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                        </a>
                                        
                                        <form action="{{ route('admin.task-bids.destroy', $bid) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus penawaran ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-8 text-center text-sm text-gray-500">
                                    Tidak ada data penawaran mitra.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
